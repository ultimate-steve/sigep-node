...
class PrintPDF {
     ...
     * - lista de postagem
     */
    constructor(idPLP, PLP, path, filename, nfUrls = []){
        this.idPLP =idPLP
        this.PLP = PLP
        this.path = path
    ...
    async _buildHTMLTags(){
        ...
        for(let i = 0; i < correioslog.objeto_postal.length; i++)
        {
            let val = correioslog.objeto_postal[i];
            val.base64 = {}
            ...
            if(this.nfUrls[i])
            {
                val.base64.nfCode = await this.helpers.generateQRCode(this.nfUrls[i].url);
                val.base64.chave_acesso = await this.helpers.generateBarcode128( this.nfUrls[i].key, 18, 40 )
                val.nacional.chave_acesso = this.nfUrls[i].key;
                val.nacional.cnpj = this.nfUrls[i].cnpj;
                val.nacional.cliente = this.nfUrls[i].customer;
                val.nacional.protocolo_autorizacao = this.nfUrls[i].protocol;
            }
        }
    ...
    async _buidHTMLar(){
        let correioslog = this.correioslog
        let data = {
            plp: correioslog.plp,
            remetente: correioslog.remetente,
            forma_pagamento: correioslog.forma_pagamento
        }

        data.ars = correioslog.objeto_postal.find(( val ) => {
            let finds = val.codigo_servico_postagem.find(( el ) => {
                return el === "001"
            })
        ...
        data.barcode = await this.helpers.generateBarcode128( correioslog.plp.id_plp, 18, 75 )
        data.idPLP = correioslog.plp.id_plp

        let buffLogo = fs.readFileSync( pathLocal + '/assets/img/logo-correios.png' )
        data.logo = Buffer.from( buffLogo ).toString( 'base64' )

        let renderFile = promisify(ejs.renderFile)
        let html = await renderFile(pathLocal + '/assets/html/ar.ejs', {
            ...data
        }, {
            async: true,
            client: true
    ...
    //async arToPDF( pathOut, dateClosed = null ){
        //const browser = await puppeteer.launch()
        //const html = await this._buidHTMLar( date )
        ...
        ////adiciona css
        //await page.addStyleTag({
            //path: pathLocal + '/assets/css/ar.css'
        //})
